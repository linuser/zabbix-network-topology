...
function section(parent, label) {
    parent.appendChild(el('div',
        ...
        + 'color:var(--nt-muted,#94a3b8);margin:9px 0 4px;border-top:1px solid var(--nt-line-soft,#f1f5f9);padding-top:6px', label));
}
...
function row(parent, label, value, valueEl) {
    ...
    r.appendChild(el('span', 'color:var(--nt-muted,#94a3b8);min-width:88px;flex-shrink:0', label));
    if (valueEl) { valueEl.style.color = valueEl.style.color || '#475569'; r.appendChild(valueEl); }
    else         { r.appendChild(el('span', 'color:var(--nt-text-2,#475569)', value)); }
...
function hint(parent, text) {
    parent.appendChild(el('div',
        'font-size:10px;color:var(--nt-warn-text,#92400e);background:rgba(245,158,11,0.10);border-radius:5px;'
...
export function showEdgeDetail(panel, ed) {
    ...
    const title = el('div',
        'flex:1;min-width:0;font-size:13px;font-weight:600;color:var(--nt-text,#0f172a);'
        + 'overflow:hidden;text-overflow:ellipsis;white-space:nowrap');
    title.appendChild(el('span', '', sLbl));
    title.appendChild(el('span', 'color:var(--nt-muted,#94a3b8);margin:0 4px', '↔'));
    title.appendChild(el('span', '', tLbl));
    head.appendChild(title);

    const close = el('button',
        'background:none;border:none;cursor:pointer;color:var(--nt-muted,#94a3b8);font-size:18px;'
    ...
    if (!srcRow.childNodes.length) srcRow.appendChild(el('span', 'color:var(--nt-muted,#94a3b8)', '—'));
    ...
    if (d.portMatch === 'normalized') {
        panel.appendChild(el('div', 'font-size:10px;color:var(--nt-muted,#94a3b8);line-height:1.4;margin-top:3px',
    ...
    if (!tIn && !tOut && !cap) {
        panel.appendChild(el('div', 'font-size:11px;color:var(--nt-muted,#94a3b8)', t('edge.traffic.none')));
    } else {
        const live = el('div',
        ...
        if (cap > 0) {
            ...
            v.appendChild(el('span', 'color:var(--nt-muted,#94a3b8)', t('edge.of', { cap: fmt(cap) })));
            row(panel, t('edge.util'), null, v);
        } else {
            row(panel, t('edge.util'), null, el('span', 'color:var(--nt-muted,#94a3b8)', t('edge.cap.unknown')));
        }
    ...
    if (pErr !== null || pDrop !== null) {
        ...
        [[t('edge.errors'), pErr, ERR_THRESHOLD], [t('edge.discards'), pDrop, DROP_THRESHOLD]]
            .forEach(function(m) {
                ...
                v.appendChild(document.createTextNode(' '));
                v.appendChild(el('span', 'color:var(--nt-muted,#94a3b8)', t('edge.threshold', { n: m[2] })));
    ...
    if (down || errs || drops) {
        ...
        if (down) {
            ...
            v.appendChild(document.createTextNode(' '));
            v.appendChild(el('span', 'color:var(--nt-muted,#94a3b8)', '(' + Math.round(ratio * 100) + '%)'));
