...
import { seiteIstDunkel } from './utils.js';
...
export function initDarkMode() {
    ...
    // ZWEI WEGE, und der gemessene gewinnt.
    //
    // detectZabbixDark() erkennt das Theme am DATEINAMEN des eingebundenen
    // Stylesheets. Das stimmt fuer die vier Themes, die Zabbix heute
    // ausliefert, und hat zwei Luecken: Zabbix 8.0 bringt mit ZBXNEXT-10657
    // ein weiteres dunkles Theme ("Dark blue theme"), das dort nicht steht —
    // und wer eigenes Theme-CSS ausliefert, nennt es "corporate" und faehrt
    // trotzdem schwarz. In beiden Faellen stuende die Karte hell in einem
    // dunklen Zabbix.
    //
    // seiteIstDunkel() fragt deshalb die Seite selbst: es misst die
    // tatsaechliche Hintergrundfarbe und rechnet die relative Helligkeit nach
    // WCAG aus. Das kennt keine Namen und irrt sich deshalb auch nicht an
    // ihnen. Nur wenn sich nichts messen laesst — durchsichtiger Hintergrund,
    // oder getComputedStyle wirft — bleibt der Dateiname als Rueckfall.
    if (root) root.classList.toggle('nt-dark', seiteIstDunkel(detectZabbixDark()));
}
