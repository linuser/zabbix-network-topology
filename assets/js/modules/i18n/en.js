...
export default {
    ...
    'table.search.placeholder': 'Search — host:web, type:switch, port:Gi1/0/8, ...',
    'table.csv_tip':          'Download the visible rows as CSV — filtered and sorted as shown, device and port in their own columns',
    ...
    'table.col.uplink':       'Connected to',
    'table.uplink.more':      '+{n} more',
    'table.uplink.stale':     'last seen here',
    ...
    'toolbar.cluster.needs_groups': 'Only does something with two or more host groups on the map \u2014 there is nothing to separate here.',
    ...
    'err.timeout':   'The request took too long ({code}). The selection is probably too large — fewer hops, or a smaller host group.',
    'err.session':   'Not signed in any more ({code}). Reload the page.',
    'err.http':      'The server answered with {code}.',
    'err.not_json':  'The server did not answer with data (it began with: {start}). Usually a timeout or an error page in front of Zabbix.',
    'warn.scope_cut':'Only {done} of {asked} hops shown — {hosts} devices is the limit for this view. Fewer hops, or use the host groups.',
    ...
    'detail.ghost.status': 'NOT MONITORED',
    'detail.ghost.seen_via': 'Seen via',
    'detail.ghost.seen_by': 'Reported by',
    'detail.ghost.caps': 'Capabilities',
    'detail.ghost.desc': 'Vendor/model',
    'detail.ghost.hint': 'No Zabbix host, so no metrics. Everything above comes from the neighbour that reported it.',
    ...
    'edge.metric.why_estimate': 'Ports are reported, but no interface counters could be matched to them. Two things have to hold: lldpRemLocalPortNum equals ifIndex, which is true on some vendors and not on others, and the host carries interface items for that index. The figures above are therefore an estimate from the host totals, not this port.',
    ...
    'edge.confirmed.tip': 'Both ends report each other, so the link is confirmed from both sides.',
    ...
    'edge.confidence.tip': 'How sure this link is: {how} Both ends reporting each other, a second protocol, and known ports each raise it.',
    'edge.confidence.tip_plain': 'How sure this link is. Both ends reporting each other, a second protocol, and known ports each raise it.',
    ...
    'edge.match.alias': 'the reported name is declared on the host with nt:lldp \u2014 the name it uses on the wire, not the one it has in Zabbix.',
    ...
    'edge.src.stale': 'no longer reported',
    'edge.src.stale.tip': 'This link was reported before but not in the latest poll. It stays on the map for a while so a single missed discovery does not make the topology jump.',
    'edge.src.stale.tip_min': 'Last reported about {n} min ago. It stays on the map for a while so a single missed discovery does not make the topology jump.',
    'edge.src.fresh': 'new',
    ...
    'tip.ghost.seen_via': 'Via',
    'tip.ghost.seen_by': 'From',
    'tip.ghost.edge': 'Leads to a device that is not a Zabbix host \u2014 nothing is measured here.',
    'tip.ghost.hint': 'Not monitored — no metrics of its own.',
    ...
    'items.needs_groups': 'The item view needs at least one host group. In host + hop-radius mode there is none — switch to Hosts, or pick a group in the filter.',
    ...
    'diag.update.check':      'Check for updates',
    'diag.update.idle':       'Asks github.com once, when you press the button — never on its own.',
    'diag.update.checking':   'Asking github.com \u2026',
    'diag.update.available':  '{v} is out.',
    'diag.update.you_have':   'You are running {v}.',
    'diag.update.current':    '{v} \u2014 you are on the latest release.',
    'diag.update.unreachable':'github.com could not be reached from this server.',
