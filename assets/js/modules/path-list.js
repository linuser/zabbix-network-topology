...
export function showPathList(panel, cy) {
    ...
    head.appendChild(el('span',
        'font-size:10px;color:var(--nt-muted,#94a3b8);flex-shrink:0',
    ...
    path.nodeIds.forEach(function(nid, i) {
        ...
        nodeRow.appendChild(el('span', 'color:var(--nt-text,#0f172a);font-weight:600', lbl));
        ...
        const linkRow = el('div',
            ...
            + 'border-left:2px solid var(--nt-faint,#cbd5e1);padding:2px 0 2px 8px;color:var(--nt-text-2,#475569)');
        ...
        if (pS || pT) {
            linkRow.appendChild(el('span', 'color:var(--nt-sub,#64748b)', (pS || '?') + ' → ' + (pT || '?')));
        }

        const pct = utilizationPct(d);
        if (pct !== null) {
            if (linkRow.childNodes.length) linkRow.appendChild(el('span', 'color:var(--nt-faint,#cbd5e1)', '·'));
            linkRow.appendChild(el('b', 'color:' + utilizationColor(pct), pct.toFixed(1) + '%'));
            if (!d.perLink) linkRow.appendChild(el('span', 'color:var(--nt-muted,#94a3b8);font-size:10px', t('path.est')));
        ...
        if (!linkRow.childNodes.length) {
            linkRow.appendChild(el('span', 'color:var(--nt-muted,#94a3b8)', t('path.no_data')));
        }
