...
> - **Two new host tags are available:** `nt:uplink=<host>:<port>` for devices that cannot report a neighbour, and `nt:lldp=<name>` for hosts whose name on the wire differs from their name in Zabbix. Nothing changes if you never set them.
...
> - **Zwei neue Host-Tags stehen bereit:** `nt:uplink=<host>:<port>` für Geräte, die keinen Nachbarn melden können, und `nt:lldp=<name>` für Hosts, deren Name auf dem Draht von dem in Zabbix abweicht. Wer sie nie setzt, merkt nichts davon.
